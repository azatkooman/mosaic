...
--
-- Organizers needed a way to stop registrations *now* — a venue fills, a
-- deadline slips — without editing registration_closes_at and losing the real
-- date they published. This adds a switch that sits alongside the window
-- rather than replacing it: either one being shut closes registration.
--
-- Both write paths are recreated to honour it, because a UI-only gate would be
-- bypassable by any direct POST:
--   * submit_registration    — copied verbatim from 0031 (the current version;
--                              0015 and 0005 are superseded) + one clause.
--   * update_own_participant — copied verbatim from 0027 + the same clause, so
--                              a manual close also freezes self-service edits,
--                              exactly as registration_closes_at already does.
--
-- The exception message is deliberately the EXISTING 'registration is closed'
-- string: app/api/register/route.js already maps it to a 409, so the route
-- needs no change and the registrant sees the right message in their language.
...
comment on column events.registration_manually_closed is
  'Organizer kill-switch for registration, independent of registration_closes_at. Either being shut closes registration.';

-- --- submit_registration (from 0031) ----------------------------------------
...
-- --- update_own_participant (from 0027) -------------------------------------
