...
/** Country calling codes (E.164). iso = ISO 3166-1 alpha-2, dial = code without '+'. */
const COUNTRIES = [
...
/** For dial codes shared by several countries, which one a stored value maps to. */
const SHARED_DIAL_DEFAULT = { 1: 'US', 7: 'RU' }

const LOCALE_DEFAULT_ISO = { en: 'US', es: 'ES', fr: 'FR', ru: 'RU', uk: 'UA' }
...
/** Split a stored "+66 812345678" back into country + national number. */
function parseValue(value) {
  ...
  for (const c of COUNTRIES) {
    ...
    if (match && c.dial.length <= match.dial.length) continue
    match = c
  }
  if (!match) return null
  const preferred = SHARED_DIAL_DEFAULT[match.dial]
  if (preferred) match = BY_ISO.get(preferred)
  return { iso: match.iso, national: digits.slice(match.dial.length).trim() }
}

/** Phone control: country-code select + national number input, emitting
 *  a single "+<dial> <number>" string (or '' when the number is empty). */
export function PhoneInput({ id, describedBy, invalid, value, onChange, maxLength, locale }) {
  const [iso, setIso] = useState(
    () => parseValue(value)?.iso ?? LOCALE_DEFAULT_ISO[locale] ?? 'US'
  )
  const dial = BY_ISO.get(iso).dial
  const parsed = parseValue(value)
  const national = parsed ? parsed.national : (value ?? '')

  const emit = (nextDial, nextNational) => {
    const n = nextNational.replace(/^\+/, '').trim()
    ...
    <div className="phone-row">
      <NativeSelect
        ...
        onChange={(e) => {
          ...
          if (national) emit(BY_ISO.get(e.target.value).dial, national)
        }}
      >
        {COUNTRIES.map((c) => (
