 ...
 * auto-translated.
 *
 * Seven strings on the event page have platform defaults (the Register button,
 * the countdown label and its four unit labels, the View-agenda button). The
 * page renders `lt(map, contentLocale, defaultLocale) || t(key)` for each, so
 * an untouched event falls through to the `event` i18n namespace — which only
 * covers the five platform locales. An event offered in an organizer-added
 * language (Thai, Tagalog, …) therefore showed those seven strings in the
 * visitor's UI language while everything around them was translated.
 *
