...
/**
 * Swap `{event}` for `{0}` before sending a string to the translator.
 *
 * The words inside the braces are the problem. Asked for Arabic or Cantonese,
 * Google translates them too — `"Register for {event}"` comes back with the
 * placeholder renamed — and `placeholdersIntact` then has to throw the whole
 * string away, so the label falls back to English. Observed live: the
 * single-placeholder `wizard.title` failed for 2 of 7 languages and the
 * two-placeholder `wizard.participantOf` for 6 of 7, the failure rate
 * compounding per placeholder exactly as a per-placeholder mangling would.
 *
 * A digit has nothing to translate, which removes the cause rather than
 * detecting it. Restoring BY INDEX also means a language that MOVES the
 * placeholder is handled correctly instead of merely tolerated — Kazakh puts it
 * first (`{event}-ке тіркеліңіз`), and Arabic reorders — because `{1}` maps back
 * to whichever name it started as, wherever it lands.
 *
 * @param {string} text
 * @returns {{masked: string, names: string[]}}
 */
export function protectPlaceholders(text) {
  const names = []
  const masked = String(text ?? '').replace(/\{\s*([a-zA-Z0-9_]+)\s*\}/g, (_, name) => {
    names.push(name)
...
/**
 * Put the real placeholder names back after translation.
 *
 * Returns null when a token did not survive — dropped, renamed, or with an index
 * we never issued. The caller then treats the string as untranslated, so it
 * falls back to the route locale and is retried on the next run rather than
 * being stored broken.
 *
 * @param {string} text
 * @param {string[]} names  from `protectPlaceholders`
 * @returns {string|null}
 */
export function restorePlaceholders(text, names) {
  if (typeof text !== 'string') return null
  if (!names?.length) return text
  const seen = new Set()
  const out = text.replace(/\{\s*(\d+)\s*\}/g, (match, digits) => {
    const index = Number(digits)
    // An index we never issued means the translator invented a token; leave it
    // in place so the count check below fails and the string is discarded.
    if (!Number.isInteger(index) || index < 0 || index >= names.length) return match
    seen.add(index)
    return `{${names[index]}}`
  })
  return seen.size === names.length ? out : null
}
