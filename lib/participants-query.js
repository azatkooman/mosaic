...
export function applyParticipantFilters(q, f = {}, questions = []) {
  ...
  for (const [qid, value] of Object.entries(answerFilters ?? {})) {
    ...
    if (question.type === 'multiselect') {
      // A respondent can tick several options, so the filter takes several too.
      // The chosen semantics is AND — rows that selected EVERY ticked option —
      // which one jsonb containment expresses exactly: `answers @> {"q":["a","b"]}`
      // is true only when the stored array holds both. That also keeps the whole
      // filter in a single `.contains()` rather than an OR of clauses, which
      // would have to embed JSON in `.or()` syntax or fall back to the
      // substring/underscore-wildcard `ilike` the search uses — acceptable when
      // fuzzy-matching a typed word, not for an exact filter.
      const wanted = (Array.isArray(value) ? value : String(value).split(','))
        .map((v) => String(v ?? '').trim())
        .filter(Boolean)
      if (wanted.length === 0) continue
      q = q.contains('answers', { [qid]: wanted })
      ...
      // altogether. `not.cs` covers both that absence and any legacy row that
      // did store false, where `answers->>qid != 'true'` would drop the absent
      // rows (SQL comparisons against NULL are never true).
      if (value === 'false') q = q.not('answers', 'cs', JSON.stringify({ [qid]: true }))
      else q = q.contains('answers', { [qid]: true })
