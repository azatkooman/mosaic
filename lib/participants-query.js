...
export function applyParticipantFilters(q, f = {}, questions = []) {
  ...
  for (const [qid, raw] of Object.entries(answerFilters ?? {})) {
    ...
    if (!question) {
      // The filter names a question this list does not have. That happens for
      // real: an xlsx exported from the All tab writes a sheet per bucket, and
      // the All tab's filters may name questions only the group form asks — on
      // thai-ccci, "Your passport scan is not empty" is meaningless to the
      // individual list, which has no such question.
      //
      // Skipping it, as this did, does not narrow that sheet — it WIDENS it,
      // dropping the condition and returning rows the organizer had filtered
      // out. A condition that cannot be evaluated here cannot be satisfied
      // here, so the list is empty instead. `.in('id', [])` is the same
      // no-rows idiom the empty bucket uses above.
      //
      // A filter carrying no value is not a condition at all and is still
      // ignored, so an emptied control never blanks a sheet. A junk id from a
      // hand-edited URL now yields an empty file rather than an unfiltered
      // one, which is the safer way for that to fail.
      const carriesValue = raw !== '' && raw != null && !(Array.isArray(raw) && raw.length === 0)
      if (carriesValue) q = q.in('id', [])
      continue
    }
