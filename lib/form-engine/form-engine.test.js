...
describe('evaluateRule', () => {
  ...
  it('treats an untouched composite answer as empty', () => {
    // name / phone / address answers are objects. These used to count as
    // filled no matter what, so isEmpty never matched on them — and presence
    // is the only thing a rule can test against a composite.
    const blank = [
      {},
      { first: '', last: '' },
      { code: '', number: '' },
    ...
    for (const v of blank) {
      expect(evaluateRule(v, { questionId: 'x', operator: 'isEmpty' }), JSON.stringify(v)).toBe(true)
      expect(evaluateRule(v, { questionId: 'x', operator: 'isNotEmpty' })).toBe(false)
    }
    const filled = { first: 'Ada', last: '' }
    expect(evaluateRule(filled, { questionId: 'x', operator: 'isNotEmpty' })).toBe(true)
    expect(evaluateRule(filled, { questionId: 'x', operator: 'isEmpty' })).toBe(false)
  })
})
...
describe('validateAnswer', () => {
  ...
  it('enforces minLength', () => {
    const q = { id: 'q', type: 'text', validation: { minLength: 3 } }
    expect(validateAnswer(q, 'abc')).toBe(null)
    expect(validateAnswer(q, 'ab')).toBe('tooShort')
    // An empty optional answer is unanswered, not too short.
    expect(validateAnswer(q, '')).toBe(null)
  })

  it('applies patterns in unicode mode so non-Latin scripts pass', () => {
    const q = { id: 'q', type: 'text', validation: { pattern: "^[\\p{L}\\s'’-]+$" } }
    expect(validateAnswer(q, 'Олена')).toBe(null)
    expect(validateAnswer(q, 'Ada42')).toBe('invalid')
  })

  it('ignores a pattern that cannot compile rather than blocking the registrant', () => {
    const q = { id: 'q', type: 'text', validation: { pattern: '^([unclosed' } }
    expect(validateAnswer(q, 'anything')).toBe(null)
  })

  it('still honours a pattern that is only valid without the unicode flag', () => {
    // `\-` outside a class is an error under /u; the fallback compile keeps
    // rules authored before presets existed working.
    const q = { id: 'q', type: 'text', validation: { pattern: '^a\\-b$' } }
    expect(validateAnswer(q, 'a-b')).toBe(null)
    expect(validateAnswer(q, 'axb')).toBe('invalid')
  })
})
