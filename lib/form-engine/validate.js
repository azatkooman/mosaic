...
/**
 * Compile a definition-supplied pattern. Unicode mode first, so `\p{L}` works
 * — an ASCII-only "letters" rule would reject Cyrillic names on a platform
 * that ships ru/uk. Patterns that are only valid WITHOUT the flag (a stray
 * `\-`, say) still compile on the second pass, so no stored rule regresses.
 * Returns null for a pattern that is broken either way; callers skip the check
 * rather than block the registrant on an authoring mistake.
 */
...
export function validateAnswer(q, value) {
  ...
  switch (q.type) {
    ...
    case 'textarea': {
      ...
      if (v.pattern) {
        const re = compilePattern(v.pattern)
        // A broken pattern must not block the registrant.
        if (re && !re.test(value)) return 'invalid'
      }
      return null
    }
    case 'email':
 ...
 * @param {{audience?: 'registrant'|'admin'}} [options]  'admin' also validates
 *   admin-only questions; the default excludes them, which is what keeps a
 *   crafted /api/register payload from setting one.
...
export function validateParticipantAnswers(
  ...
  const visible = visibleQuestions(definition, participantTypeKey, answers, options)
