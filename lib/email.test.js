...
describe('sendEmail without SMTP configured', () => {
  test('degrades to false instead of throwing', async () => {
    // Production still has no SMTP_* vars, so every send takes this path. A
    // throw here would turn a committed status change into a failed request.
    const { sendStatusChangeEmail } = await import('./email')
