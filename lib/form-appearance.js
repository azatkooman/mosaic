...
export function resolveFormAppearance(appearance, eventTheme) {
  ...
  return {
    theme,
    // Includes `header.bg_image_path`, the storage path for the header band's
    // background image. Not inherited from the event page the way the theme
    // keys above are: an organizer who wants the event's cover there says so
    // with the panel's "use the event cover" button, which copies the path in.
    // Inheriting it silently would put a hero photo behind every form header on
    // every event that has one, uninvited.
    header: own.header ?? {},
    intro: own.intro ?? {},
