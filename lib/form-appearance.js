...
/* The platform's own values for the families a form can re-theme, used only by
   the isolating reset below. Mirrors of what fonts.css and tokens.css declare —
   mode-independent, unlike the brand colours, which need `--base-*` twins. */
const BASE_FONT_BODY = '"IBM Plex Sans"'
const BASE_FONT_DISPLAY = '"Unbounded"'
const BASE_CONTAINER_NARROW = '44rem'
 ...
 */
export function appearanceVars(resolved, { isolate = false } = {}) {
  const t = resolved?.theme ?? {}
  const vars = {}
  ...
  if (isolate) resetUnsetFamilies(vars, t)

  return vars
}

/**
 * Put back the platform's own value for every family this form does not set.
 *
 * Needed only where one themed element is nested inside another — the register
 * page themes the whole column from the form a reader will land on, and the
 * wizard re-themes the panel from the form that actually applies once a mode is
 * picked. Custom properties inherit, so a key the inner form leaves unset kept
 * the OUTER form's value: a single form with no font of its own wore the
 * default form's, its corners took the default form's radius, its buttons took
 * the default form's brand colour. Two forms' customizations at once, which is
 * exactly what it looked like.
 *
 * Only the families that are independent of what is behind them. `--ink`,
 * `--surface` and the rest are deliberately left to inherit: a form that sets
 * no background of its own is genuinely sitting on the outer form's background,
 * so the outer form's ink is the right ink for it, and resetting those would
 * put dark text back on a dark page.
 */
function resetUnsetFamilies(vars, t) {
  if (!t.primary_color) {
    // Via twins in tokens.css rather than literals: the brand colour differs
    // between light and dark mode, and no value written here could be right in
    // both.
    vars['--pine'] = 'var(--base-pine)'
    vars['--pine-deep'] = 'var(--base-pine-deep)'
    vars['--pine-tint'] = 'var(--base-pine-tint)'
    vars['accentColor'] = 'var(--base-pine)'
  }
  if (!FONT_FAMILIES[t.body_font]) vars['--font-body'] = BASE_FONT_BODY
  if (!FONT_FAMILIES[t.title_font]) vars['--font-display'] = BASE_FONT_DISPLAY
  if (!SCALES.text[t.text_scale] || SCALES.text[t.text_scale] === 1) {
    for (const [name, rem] of Object.entries(BASE_TEXT)) vars[name] = `${rem}rem`
  }
  if (!SCALES.density[t.density] || SCALES.density[t.density] === 1) {
    for (const [name, rem] of Object.entries(BASE_SPACE)) vars[name] = `${rem}rem`
  }
  const radiusFactor = SCALES.radius[t.radius]
  if (radiusFactor == null || radiusFactor === 1) {
    for (const [name, px] of Object.entries(BASE_RADIUS)) vars[name] = `${px}px`
  }
  if (!WIDTHS[t.width]) vars['--container-narrow'] = BASE_CONTAINER_NARROW
}
