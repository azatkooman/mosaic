...
// Node's and the browsers' ICUs disagree on the whitespace inside formatted
// dates (thin space U+2009 / narrow no-break U+202F / NBSP vs plain space —
// e.g. around formatRange's en dash), so the same call yields different
// strings on server and client and React hydration fails. Normalize every
// Intl-produced string to plain spaces.
const stableSpaces = (s) => s.replace(/[\u00a0\u2009\u202f]/g, ' ')
...
export function formatEventDate(iso, timeZone, locale, opts = {}) {
  ...
  if (dateFormat === 'auto') {
    ...
    return fmt ? stableSpaces(fmt.format(date)) : date.toISOString()
  }
  ...
  return stableSpaces(`${assembleDate(get, dateFormat)}, ${assembleTime(get, hour12)}`)
}
...
export function formatEventDateRange(startIso, endIso, timeZone, locale, prefs = {}) {
  ...
  if (dateFormat === 'auto') {
    ...
    return stableSpaces(endIso ? fmt.formatRange(start, new Date(endIso)) : fmt.format(start))
  }
...
export function formatLocalInput(value, locale, prefs = {}) {
  ...
  if (dateFormat === 'mdy') dateText = `${mo}/${d}/${y}`
    ...
    ? stableSpaces(new Intl.DateTimeFormat(locale, { dateStyle: 'medium' }).format(local))
    : `${y}-${mo}-${d}`
  ...
  if (timeFormat === 'h24') timeText = `${String(hStr).padStart(2, '0')}:${minStr}`
    ...
    ? stableSpaces(new Intl.DateTimeFormat(locale, { hour: 'numeric', minute: '2-digit' }).format(local))
    : `${hStr}:${minStr}`
...
export function formatSampleTime(timeFormat, locale) {
  if (timeFormat === 'auto') {
    ...
    return fmt ? stableSpaces(fmt.format(new Date(SAMPLE_ISO))) : '14:30'
  }
  const hour12 = timeFormat === 'h12'
  const get = forcedParts(new Date(SAMPLE_ISO), 'UTC', locale, { withTime: true, hour12 })
  return get ? stableSpaces(assembleTime(get, hour12)) : '14:30'
}
