 ...
 * @returns {string}
 */
export function eventPageUrl({ slug, code, uiLocale = 'en', origin = '', subPath = '' }) {
  const base = origin || ''
  // A platform locale owns its own route; anything else is a custom language.
  if (code && LOCALES.includes(code)) {
    return `${base}/${code}/events/${slug}${subPath}`
  }
  const routeLocale = LOCALES.includes(uiLocale) ? uiLocale : 'en'
  const path = `${base}/${routeLocale}/events/${slug}${subPath}`
  return code ? `${path}?lang=${encodeURIComponent(code)}` : path
}
